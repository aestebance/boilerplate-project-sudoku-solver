...
suite('UnitTests', () => {
    suite("Valid tests", () =>{
        ...
        test("Logic handles a puzzle string with invalid characters (not 1-9 or .)", function(done){
            const puzzleString = "1.5..2.84..63.12.7.2..5.a...9..1.c..8.2.3674.3.7.2..9.47.f.8..1..16....926914.37.";
            assert.deepEqual(solver.validate(puzzleString), { error: 'Invalid characters in puzzle' } );
            assert.deepEqual(solver.validate(puzzleString), { error: 'Invalid characters in puzzle' } );
            done();
        });
        test("Logic handles a puzzle string that is not 81 characters in length", function(done) {
            const puzzleString = "1.5..2.84..63.12..2..5....9..1....8.2.3674.3.7.2..9.47...8..1..16....926914.37."
    ...
    suite("Solver testing", () => {
        ...
        test("Solver returns the the expected solution for an incomplete puzzzle", function(done){
           ...
           assert.equal(solver.solve(puzzleString), "568913724342687519197254386685479231219538467734162895926345178473891652851726943");
           done();
        });
    });
});
